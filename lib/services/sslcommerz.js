...
export const sslcommerz = {
  ...
  validate: async (data) => {
     const baseUrl = is_live ? 'https://securepay.sslcommerz.com' : 'https://sandbox.sslcommerz.com'
     const validateUrl = `${baseUrl}/validator/api/validationserverAPI.php?val_id=${encodeURIComponent(data.val_id)}&store_id=${encodeURIComponent(store_id)}&store_passwd=${encodeURIComponent(store_passwd)}&format=json&v=1`
